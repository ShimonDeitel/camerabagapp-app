...
final class CamerabagUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() {
        for i in 0..<(Self.freeLimitForUITest + 1) {
            ...
            if nameField.waitForExistence(timeout: 2) {
                ...
                app.buttons["entrySaveButton"].tap()
            } else if app.staticTexts["Camerabag Pro"].waitForExistence(timeout: 2) {
                break
            }
        }
        XCTAssertTrue(app.staticTexts["Camerabag Pro"].waitForExistence(timeout: 3))
    }
